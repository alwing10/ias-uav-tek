...
// Только КОНКРЕТНЫЕ типы объектов ТЭК.
// «other» fallback убран — раньше пропускал общие новости про
// нефть/газ/энергетику (например, цены или ремонт), что не имеет
// отношения к инцидентам с БПЛА.
const OBJECT_PATTERNS = [
  ...
  { type: 'oil_pipeline', name: 'Нефтепровод (по сообщению)', regex: /нефтепровод|друж[а-я]*\s*нефт|транснефт/i },
  ...
  { type: 'power_grid', name: 'ЛЭП (по сообщению)', regex: /лэп\b|линия\s+электропередач/i },
  { type: 'substation', name: 'Подстанция (по сообщению)', regex: /подстанц/i },
  { type: 'power_plant', name: 'Электростанция (по сообщению)', regex: /тэц|тэс\b|грэс|аэс\b|электростанц/i },
];
