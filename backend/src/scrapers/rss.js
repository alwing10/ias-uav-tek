...
const FEEDS = [
  ...
  // Тематические запросы по типу объектов
  ...
  gnews('БПЛА газокомпрессорная станция', 'GN9'),
  gnews('дрон ТЭЦ ГРЭС электростанция', 'GN10'),
  gnews('беспилотник ЛЭП линия электропередач', 'GN11'),
  gnews('атака БПЛА газопровод', 'GN12'),
  gnews('Герань-2 удар нефть', 'GN13'),
  // Региональные запросы
  gnews('атака БПЛА Белгород', 'GN6'),
  gnews('атака БПЛА Краснодар', 'GN7'),
  gnews('атака БПЛА Ростов', 'GN8'),
  gnews('атака БПЛА Воронеж нефть', 'GN14'),
  gnews('БПЛА Брянск нефтебаза', 'GN15'),
  gnews('БПЛА Самара Саратов НПЗ', 'GN16'),
  gnews('БПЛА Татарстан нефть', 'GN17'),
  gnews('БПЛА Курск энергетика', 'GN18'),
  // Англоязычные — для GDELT-fallback
  gnews('Russia drone strike refinery', 'GN19'),
  gnews('Ukraine UAV oil depot Russian', 'GN20'),
...
// Жёсткий предфильтр: пропускаем только новости, где УПОМЯНУТ БПЛА.
// (parser.js делает финальную проверку наличия объекта ТЭК — там тоже строго).
const HAS_UAV_KEYWORD = /бпла|беспилотн|дрон|fpv|фпв|шахед|shahed|герань|loitering|kamikaze|uav\b|квадрокоптер/i;
const RELEVANT_KEYWORDS = HAS_UAV_KEYWORD;
